...
const intialState = {
  topRight: [
    { number: 8, color: '#518EA1' },
    { number: 7, color: '#518EA1' },
    { number: 6, color: '#518EA1' },
    { number: 5, color: '#518EA1' },
    { number: 4, color: '#518EA1' },
    { number: 3, color: '#518EA1' },
    { number: 2, color: '#518EA1' },
    { number: 1, color: '#518EA1' },
  ],
  all: false,
};
...
class TopRight extends Component {
  ...
  async renderSelect(item, index, position) {
    if (item.color == '#518EA1') {
      await this.setState(prevState => {
        const newItems = [...prevState.topRight];
        newItems[index].color = '#f4b841';
        return { topRight: newItems };
      })
    } else {
      await this.setState(prevState => {
        const newItems = [...prevState.topRight];
        newItems[index].color = '#518EA1';
        return { topRight: newItems };
      })
    }
    let teeth = []
    for (let item of this.state.topRight) {
      if (item.color == '#f4b841') {
        teeth.push(item.number)
      }
    }
    this.props.topRightCallBack(teeth);
  }
  async renderSelectAll() {
    if (!this.state.all) {
      ...
      await this.setState({ topRight: teeth1, all: !this.state.all })
      let teeth = []
      for (let item of this.state.topRight) {
        if (item.color == '#f4b841') {
          teeth.push(item.number)
      ...
      <Col style={[styles.colContainer]}>
        ...
        <Row>
          <ScrollView ref={(scroll) => { this.scroll = scroll; }} horizontal={true}>
            {this.state.topRight.map((item, index) => (
              <TouchableOpacity onPress={() => this.renderSelect(item, index, 'topRight')}>
          ...
          <Col size={50}>
            <TouchableOpacity onPress={() => this.renderSelectAll()}>
              <Row>
                ...
                <Text style={{ fontSize: 12 }}>فک بالا راست</Text>
                {this.state.all ? <Icon style={{ color: 'green', fontSize: 12 }} name="ios-checkmark-outline" /> : null}
              </Row>
            </TouchableOpacity>
          </Col>
