...
class TopLeft extends Component {
  ...
  async renderSelect(item, index) {
    if (item.color == '#7EBBD1') {
      await this.setState(prevState => {
        const newItems = [...prevState.topLeft];
        newItems[index].color = '#f4b841';
        return { topLeft: newItems };
      })
    } else {
      await this.setState(prevState => {
        const newItems = [...prevState.topLeft];
    ...
    let teeth = []
    for (let item of this.state.topLeft) {
      if (item.color == '#f4b841') {
        teeth.push(item.number)
      }
    }
    this.props.topLeftCallBack(teeth);
  }
  async renderSelectAll() {
    if (!this.state.all) {
      ...
      await this.setState({ topLeft: teeth1, all: !this.state.all })
      let teeth = []
      for (let item of this.state.topLeft) {
        if (item.color == '#f4b841') {
          teeth.push(item.number)
      ...
      <Col style={[styles.colContainer]}>
        ...
        <Row>
          <ScrollView horizontal={true}>
            {this.state.topLeft.map((item, index) => (
          ...
          <Col size={50}>
            <TouchableOpacity onPress={() => this.renderSelectAll()}>
              <Row>
                ...
                <Text style={{ fontSize: 12 }}>فک بالا چپ</Text>
                {this.state.all ? <Icon style={{ color: 'green', fontSize: 12 }} name="ios-checkmark-outline" /> : null}
              </Row>
            </TouchableOpacity>
          </Col>
