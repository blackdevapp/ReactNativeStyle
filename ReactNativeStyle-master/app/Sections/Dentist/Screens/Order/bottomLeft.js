...
class BottomLeft extends Component {
  ...
  renderSelect(item, index) {
    if (item.color == '#518EA1') {
      this.setState(prevState => {
        const newItems = [...prevState.bottomLeft];
        newItems[index].color = '#f4b841';
        return { bottomLeft: newItems };
      })
    } else {
      this.setState(prevState => {
        const newItems = [...prevState.bottomLeft];
    ...
    let teeth = []
    for (let item of this.state.bottomLeft) {
      if (item.color == '#f4b841') {
        teeth.push(item.number)
      }
    }
    this.props.bottomLeftCallBack(teeth);
  }
  async renderSelectAll() {
    if (!this.state.all) {
      ...
      await this.setState({ bottomLeft: teeth1, all: !this.state.all })
      let teeth = []
      for (let item of this.state.bottomLeft) {
        if (item.color == '#f4b841') {
          teeth.push(item.number)
      ...
      <Col style={[styles.colContainer]}>
        ...
        <Row>
          <ScrollView horizontal={true}>
            {this.state.bottomLeft.map((item, index) => (
          ...
          <Col size={50}>
            <TouchableOpacity onPress={() => this.renderSelectAll()}>
              <Row>
                ...
                <Text style={{ fontSize: 12 }}>فک پایین چپ</Text>
                {this.state.all ? <Icon style={{ color: 'green', fontSize: 12 }} name="ios-checkmark-outline" /> : null}
              </Row>
            </TouchableOpacity>
          </Col>
