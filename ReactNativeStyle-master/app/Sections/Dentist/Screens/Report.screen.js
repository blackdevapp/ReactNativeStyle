...
import { StyleSheet, Dimensions, Image, View, FlatList,Modal, TouchableOpacity } from 'react-native'
import { Button, Text, Container, Icon, Row, Col, Card, Input, Content, Item, CardItem, Grid,Fab, Picker, Header, Left, Title, Right, Body, } from 'native-base';
...
import { getAllConnectedLabs,getOrderLineItems } from '../Redux/Actions/Order.action'; //Import your actions
...
class ReportScreen extends PureComponent {
    ...
    componentDidMount(){
        this.props.getOrderLineItems(this.state.page, 10)
    }
    renderChart() {
        // if (this.state.time) {
        return (
        ...
        return (
            <Card style={{ height: Dimensions.get('screen').height / 7 }}>
                <CardItem style={{ justifyContent: 'space-between' }}>
                    <Text style={{ fontWeight: 'bold', color: '#7fbbd2', fontSize: 12 }}> {item.patientInfo.fullName}</Text>
                    <View style={{ flexDirection: 'row' }}>
                        <Icon name='md-calendar' style={{ fontSize: 10 }} />
                        <Text style={{ paddingRight: 5, fontSize: 10 }}>{item.orderedDate}</Text>
                    </View>
                </CardItem>
                <CardItem style={{ justifyContent: 'space-between' }}>
                    <Text style={{ fontSize: 10 }}> نام لابراتوار : {item.labName} </Text>
                    <Text style={{ fontSize: 10 }}>نوع کار : {item.orderLineItems[0].product.productName} </Text>
                </CardItem>
                <CardItem style={{ justifyContent: 'space-between' }}>
                    ...
                    <Button rounded style={{ height: 25, backgroundColor: '#7fbbd2', width: Dimensions.get('screen').width / 3 }}
    ...
    renderPage() {
        if (this.state.mode == 'factor') {
                                ...
                                <Row>
                                    ...
                                    <Col size={70} style={{ alignItems: 'center', justifyContent: 'center' }}>
                                        <Item rounded style={{ backgroundColor: 'white', width: '100%' }}>
                                            <Picker
                                                placeholder='لابراتور'
                                                selectedValue={this.state.labratory}
                                                ...
                                                renderHeader={backAction =>
                                                    <Header style={{ backgroundColor: "#7fbbd2" }}>
                                                        <Left />
                                                        <Body style={{ flex: 3 }}>
                                                            <Title style={{ color: "#fff" }}>لابراتوار را انتخاب کنید</Title>
                                                        </Body>
                                                        <Right >
                                                ...
                                                <Picker.Item label="لابراتوار را انتخاب کنید" />
                                                {this.props.connectedLabs.map((item, i) => (
                                                    <Picker.Item key={i} label={item.labName} value={item} />
                                                ))}
                                            </Picker>
            ...
            return (
                ...
                <Content style={{ flex: 13, backgroundColor: '#EFF2F6' }}>
                    ...
                    >
                        <FlatList
                            data={this.props.lineItemsDoctor}
                        ...
                        />
                        <Button rounded style={{ alignSelf: 'center', marginBottom: 5, height: 40, backgroundColor: '#7fbbd2' }}
                        onPress={()=>this.comeSoon()}>
    ...
    render() {
        return (
            <Container style={styles.container}>
                ...
                <HeaderComponent title="گزارش گیری" />
                <Row style={{ zIndex: 10, flex: 0 }}>
                    <Col>
                        {/* <TouchableOpacity> */}
                        <Button onPress={() => this.changeMode('order')} style={[(this.state.mode == 'order') ? { backgroundColor: '#ffffff' } : { backgroundColor: '#818384' }, { width: '100%', borderRadius: 0, justifyContent: 'center', height: 40 }]}>
                            <Text style={[(this.state.mode !== 'order') ? { color: '#ffffff' } : { color: '#818384' }]}>سفارش کار ها</Text>
                        </Button>
                        {/* </TouchableOpacity> */}
                    </Col>
                    <Col>
                        {/* <TouchableOpacity > */}
                        <Button onPress={() => this.changeMode('factor')} style={[(this.state.mode == 'factor') ? { backgroundColor: '#ffffff' } : { backgroundColor: '#818384' }, { width: '100%', borderRadius: 0, justifyContent: 'center', height: 40 }]}>
                            <Text style={[(this.state.mode !== 'order') ? { color: '#818384' } : { color: '#ffffff' }]}>فاکتورهای‌پرداخت‌‌ شده</Text>
                        </Button>
                        {/* </TouchableOpacity> */}
                    </Col>
                </Row>
    ...
    handleRefresh = () => {
        this.props.getOrderLineItems(0, 10)
        this.setState({
          page:0
        })
      }
      onEndReached = () => {
        if(this.props.lineItemsDoctor.length>=10){
          this.props.getOrderLineItems(this.state.page+1, 10)
          this.setState({
          ...
          <View>
            <Card style={{ backgroundColor: '#eee' }}>
              ...
              <Row style={styles.row1}>
                ...
                <Col style={styles.col}>
                  <Text>
                    {this.state.order.patientInfo.fullName}
                  </Text>
                </Col>
              </Row>
                ...
                <Row style={{ flex: 1 }}>
                  <View style={{ flex: 1 }}>
                    <FlatList
                      ...
                      data={this.state.order.orderLineItems}
...
function mapStateToProps(state, props) {
    return {
        ...
        lineItemsDoctor: state.OrderReducer.lineItemsDoctor,
    }
}
